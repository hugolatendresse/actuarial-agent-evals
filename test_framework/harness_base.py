...
class UnitTestHarness(BaseTestHarness):
    ...
    def _build_full_prompt(
        ...
        lines.append(
            f"1. Work in file: {workspace_dir / self.SOLUTION_FILENAME}")
        lines.append("2. Write your code BELOW the marker line")
        lines.append(
            "3. Test your code by running it: python step_solution.py")
        lines.append("4. Fix any errors and test again")
        lines.append(
            "5. ONLY when working perfectly, add this line at the end:")
        lines.append(f"   {self.COMPLETION_MARKER}")
        lines.append("")
        lines.append(
            "WARNING: The system evaluates immediately when it sees the completion marker!")
    ...
    def run(
        ...
        if self.mode == 'auto' and not self.is_auto_mode_supported(ide_name):
            ...
            print(
                f"Auto mode is only available for: {', '.join(IDEAutomation.SUPPORTED_IDES)}")
        ...
        if single_step:
            ...
            start_idx = next((i for i, s in enumerate(
                steps) if s.step_id == start_from), None)
...
class IntegrationTestHarness(BaseTestHarness):
    ...
    def __init__(self, config: MethodConfig, mode: str = 'manual'):
        ...
        self.results_dir = self.results_base_dir / \
            f"{config.method_name}_integration"
...
"""
    ...
    def run(self, ide_name: str) -> Dict[str, Any]:
        ...
        if self.config.integration_validator:
            validation_result = self.config.integration_validator(
                workspace_dir)
        else:
        ...
        if validation_result["passed"]:
            ...
            if "total_ultimate" in validation_result:
                ...
                print(
                    f"Expected:       {validation_result['expected_ultimate']:,.2f}")
            if "total_ibnr" in validation_result:
                print(
                    f"\nTotal IBNR:     {validation_result['total_ibnr']:,.2f}")
                print(
                    f"Expected:       {validation_result['expected_ibnr']:,.2f}")
        else:
            print("✗ TEST FAILED")
