...
Usage:
  python run_tests.py --ide cline --mode auto --test-type integration
  python run_tests.py --ide cursor --mode manual --test-type unit
"""
...
def run_all_unit_tests(ide_name: str, mode: str):
    ...
    for result in results:
        ...
        print(f"{result['method']:<{header_width}}  {status:<10}  {steps:<12}  {elapsed:<10}")
    ...
    percentage = (total_steps_passed / total_steps_count * 100) if total_steps_count > 0 else 0
            ...
            if result.get("result_details") and "results" in result["result_details"]:
                ...
                for step_result in step_results:
                    if not step_result.get("passed", True):
                        print(f"  Step {step_result.get('step_id', 'unknown')}: FAILED")
