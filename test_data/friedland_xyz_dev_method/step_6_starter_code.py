...
### STEP 5: Calculate Ultimates (from reference implementation)
from chainladder import Chainladder

# Apply Chainladder method to triangle with tail-extended development
cl_model = Chainladder()
cl_result = cl_model.fit(triangle_with_tail)

# Extract ultimate values
ultimates = cl_result.ultimate_.values[0, 0, :, 0]
total_ultimate = np.nansum(ultimates)


### STEP 6: Calculate IBNR
# Write your code below to extract IBNR from cl_result.ibnr_ attribute
