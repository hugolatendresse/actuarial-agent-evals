...
"""
    ...
    try:
        ...
        expected_rates = [0.208935, 0.468041, 0.643299,
                          0.750515, 0.841924, 0.932646, 0.984192, 0.993814]

        passed = all(np.isclose(a, b, rtol=0.01, atol=0.001) for a, b in zip(
            disposal_rates_2001[:len(expected_rates)], expected_rates[:len(disposal_rates_2001)]))
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(a, b, rtol=0.01, atol=0.001)
                     for a, b in zip(rates, gt_rates))
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(a, b, rtol=0.05, atol=2)
                     for a, b in zip(counts, gt_counts))
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(a, b, rtol=0.01)
                     for a, b in zip(severities, gt_severities))
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(a, b, rtol=0.05, atol=10)
                     for a, b in zip(paid, gt_paid))
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(a, b, rtol=0.01, atol=50)
                     for a, b in zip(ultimates, gt_ultimates))
