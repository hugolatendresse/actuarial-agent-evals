...
"""
    ...
    try:
        ...
        passed = all(np.isclose(
            actual_values[i], gt_values[i], rtol=0.001) for i in range(len(gt_values)))
...
"""
    ...
    try:
        if "CURRENT_LOSS_TREND:" in output and "PROJECTED_LOSS_TREND:" in output:
            actual_current = float(output.split("CURRENT_LOSS_TREND:")[
                                   1].split("\n")[0].strip())
            actual_projected = float(output.split(
                "PROJECTED_LOSS_TREND:")[1].strip())
...
"""
    ...
    try:
        ...
        passed = all(np.isclose(
            actual_values[i], gt_values[i], atol=0.0001) for i in range(len(gt_values)))
...
"""
    ...
    if "SELECTED_PROJECTED_NON_CAT_PURE_PREMIUM:" in output:
        actual_value = float(output.split(
            "SELECTED_PROJECTED_NON_CAT_PURE_PREMIUM:")[1].strip())
...
"""
    ...
    if "CREDIBILITY_WEIGHTED_NON_CAT_PURE_PREMIUM:" in output:
        actual_value = float(output.split(
            "CREDIBILITY_WEIGHTED_NON_CAT_PURE_PREMIUM:")[1].strip())
        passed = np.isclose(actual_value, gt_value, atol=0.01)
...
"""
    ...
    if "TOTAL_INDICATED_PURE_PREMIUM:" in output:
        actual_value = float(output.split(
            "TOTAL_INDICATED_PURE_PREMIUM:")[1].strip())
        passed = np.isclose(actual_value, gt_value, atol=0.01)
