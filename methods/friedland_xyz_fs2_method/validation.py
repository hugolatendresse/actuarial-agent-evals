...
"""
    ...
    try:
        ...
        errors = []
        if not total_match:
            errors.append(f"Total count ultimate: expected {gt_total:,.2f}, got {count_total:,.2f}")
        if not ay_match:
...
"""
    ...
    try:
        ...
        passed = all(
            np.isclose(trended_counts.get(int(year), 0), gt_trended[year], rtol=0.01)
...
"""
    ...
    try:
        ...
        passed = all(
            np.isclose(trended_freq.get(int(year), 0), gt_freq[year], atol=0.0001)
