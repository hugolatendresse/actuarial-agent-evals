import subprocess
import numpy as np
from pathlib import Path
from typing import Dict, Any


def generate_integration_prompt() -> str:
    """Generate the integration test prompt for Cape Cod method."""
    prompt = []
    prompt.append("Complete Cape Cod Method Analysis")
    prompt.append("")
    prompt.append("Using the provided data files (triangle, earned premium, rate changes, and claim ratios),")
    prompt.append("perform a Cape Cod analysis with:")
    prompt.append("- Latest-2 volume weighted LDFs")
    prompt.append("- 1.05 tail factor from age 132")
    prompt.append("- 3.425% premium trend")
    prompt.append("- Tort reform adjustments (effective 1/1/2006: -10.7% in AY 2006, -33% in AY 2007+)")
    prompt.append("")
    prompt.append("Store the final results in variables:")
    prompt.append("- total_cc_ultimate: total Cape Cod ultimate claims")
    prompt.append("- total_cc_ibnr: total Cape Cod IBNR reserves")
    prompt.append("")
    prompt.append("TESTING WORKFLOW:")
    prompt.append("1. Write your code below the marker line in integration_solution.py")
    prompt.append("2. Test by running: python integration_solution.py")
    prompt.append("3. Debug and fix any errors")
    prompt.append("4. ONLY when working perfectly, add this exact line at the end:")
    prompt.append("   ## SOLUTION COMPLETE - TESTED AND WORKING")
    prompt.append("")
    prompt.append("WARNING: The system evaluates immediately when it sees the completion marker!")
    
    return "\n".join(prompt)


def validate_integration(workspace_dir: Path, ground_truth: Dict[str, Any]) -> Dict[str, Any]:
    """Validate the integration test solution for Cape Cod method."""
    project_root = Path.cwd()
    
    validation_code = f"""
import sys
import os

os.chdir(r'{project_root}')
sys.path.insert(0, '.')

with open(r'{workspace_dir / "integration_solution.py"}', 'r') as f:
    solution_code = f.read().replace('## SOLUTION COMPLETE - TESTED AND WORKING', '')

exec(solution_code)

import numpy as np

assert 'total_cc_ultimate' in dir(), "Variable 'total_cc_ultimate' not found"
assert 'total_cc_ibnr' in dir(), "Variable 'total_cc_ibnr' not found"

print(f"TOTAL_CC_ULTIMATE: {{total_cc_ultimate:.2f}}")
print(f"TOTAL_CC_IBNR: {{total_cc_ibnr:.2f}}")
"""
    
    with open(workspace_dir / "validate.py", 'w') as f:
        f.write(validation_code)
    
    venv_python = Path("venv/bin/python")
    python_cmd = str(venv_python.absolute()) if venv_python.exists() else "python"
    
    result = subprocess.run(
        [python_cmd, "validate.py"],
        cwd=workspace_dir,
        capture_output=True,
        text=True,
        timeout=60
    )
    
    if result.returncode != 0:
        return {
            "passed": False,
            "error": f"Execution error: {result.stderr}",
            "stdout": result.stdout
        }
    
    output = result.stdout
    
    try:
        total_ultimate = None
        total_ibnr = None
        
        for line in output.split('\n'):
            if 'TOTAL_CC_ULTIMATE:' in line:
                total_ultimate = float(line.split(':')[1].strip())
            elif 'TOTAL_CC_IBNR:' in line:
                total_ibnr = float(line.split(':')[1].strip())
        
        if total_ultimate is None or total_ibnr is None:
            return {
                "passed": False,
                "error": "Could not parse total_cc_ultimate or total_cc_ibnr from output",
                "stdout": output
            }
        
        gt_ultimate = ground_truth["step_4"]["total_cc_ultimate"]
        gt_ibnr = ground_truth["step_5"]["total_cc_ibnr"]
        
        ultimate_match = np.isclose(total_ultimate, gt_ultimate, rtol=0.01)
        ibnr_match = np.isclose(total_ibnr, gt_ibnr, rtol=0.01)
        
        errors = []
        if not ultimate_match:
            errors.append(f"Total CC ultimate: expected {gt_ultimate:,.2f}, got {total_ultimate:,.2f}")
        if not ibnr_match:
            errors.append(f"Total CC IBNR: expected {gt_ibnr:,.2f}, got {total_ibnr:,.2f}")
        
        passed = ultimate_match and ibnr_match
        
        return {
            "passed": passed,
            "errors": errors if errors else None,
            "total_cc_ultimate": total_ultimate,
            "total_cc_ibnr": total_ibnr,
            "expected_ultimate": gt_ultimate,
            "expected_ibnr": gt_ibnr
        }
        
    except Exception as e:
        return {
            "passed": False,
            "error": f"Validation error: {e}",
            "stdout": output
        }

